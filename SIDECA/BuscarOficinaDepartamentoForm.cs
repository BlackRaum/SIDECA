...
namespace SIDECA
{
    public partial class BuscarOficinaDepartamentoForm : Form
    {
        private OficinaBusiness oficinaBusiness;

        private int tipo;

        public BuscarOficinaDepartamentoForm(int tipo)
        {
            InitializeComponent();
            this.tipo = tipo;
        ...
        private void btBuscar_Click(object sender, EventArgs e)
        {
            if (tbNombreUsuario.Text!=null)
            {
                ...
                lboOficina2.Text = "Seleccione una oficina";
        ...
        private void btContinuar_Click(object sender, EventArgs e)
        {
            ...
            if (tipo==1) {    
                BuscarDepartamentoForm editar = new BuscarDepartamentoForm(codigoOficina);
                editar.Show();
                this.Hide();
            }
            if (tipo == 2)
            {
                EliminarDepartamentoForm editar = new EliminarDepartamentoForm(codigoOficina);
                editar.Show();
                this.Hide();
            }
            if (tipo == 3)
            {
                    InsertarProfesionalForm insertar = new InsertarProfesionalForm(codigoOficina);
                insertar.Show();
                this.Hide();
            }
            ...
            if (tipo == 4)
            {
                //BuscarProfesionalEliminarForm eliminar = new BuscarProfesionalEliminarForm(codigoOficina);
                //insertar.Show();
                this.Hide();
            }
